// Khởi tạo các biến menuToggle và navLinks
// document.querySelector('.menu-toggle') là đi rà cả trang web nội dung nào có class là .menu-toggle
...
menuToggle.addEventListener("click", (e) => {
  ...
  // 1. Đảo ngược trạng thái ẩn/hiện
  const newState = !isVisible;
  navLinks.setAttribute("data-visible", newState);

  // 2. Thay đổi biểu tượng (PHẦN MỚI THÊM)
  if (newState === true) {
    menuToggle.innerHTML = "✕"; // Nếu mở thì thành dấu X
  } else {
    menuToggle.innerHTML = "☰"; // Nếu đóng thì thành dấu 3 gạch
  }
  // Ngăn vòng lặp liên tiếp
  e.stopPropagation();
});

// // CẦN THÊM: Trong phần đóng menu khi click ra ngoài, cũng phải đổi lại dấu 3 gạch
